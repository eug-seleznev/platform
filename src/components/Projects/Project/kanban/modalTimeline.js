...
const ModalTimeline =({setModal,id, boardId, timelineId})=>{
    ...
    const save = () => {
        const step = timeline==="Неделя"? 7 : timeline==="Две недели"? 14 : timeline==="Три недели"? 21 : timeline==="Четыре недели"? 28 : null
        const month = step? null : timeline
        dispatch(updateTimeline(id, step, boardId, timelineId, month))
        setModal(false)
    }
        ...
        <ModalContainer>
            <div className={style.window}>
                ...
                <div className={style.window__button__row}>
                    ...
                    <button className={style.window__button} style={{backgroundColor:timeline==="Четыре недели"?'rgba(0,0,0,0.2)':'rgba(0,0,0,0)'}} value="Четыре недели" onClick={(e)=>{setTimeline(e.target.value)}}>Четыре недели</button>
                </div>
                <Light size='14'color='grey' style={{marginTop:'5px',marginBottom:'10px'}}>Несколько месяцев</Light>
                <input className={style.window__input} placeholder='Введите количество месяцев' value={typeof timeline==='sting'?null:timeline} onChange={(e)=>{setTimeline(parseInt(e.target.value))}} type='number'></input>
